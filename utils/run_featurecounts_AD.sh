#!/bin/bash
PROJECT_ROOT=./data/ATAC-seq
MYPATH=./data/
OUT_DIR=$MYPATH/count_from_sc_my_peaks_AD
ANNOT=./data/peaks.saf
...
for type in $PROJECT_ROOT/AD/; do
    for region in $type/; do
        ...
        echo $ANNOT
        ~/subread/bin/featureCounts -a $ANNOT -F SAF -p -O -M -o $OUTPUT_DIR${type}_${region}_AD.peak_countMatrix.txt $PROJECT_ROOT/ATAC_seq/${type}_${region}_AD_*.bam
        rm $OUTPUT_DIR*.tmp
    done
done
wait
